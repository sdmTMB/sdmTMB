...
- Fix bug in cross validation with the new covariate diffusion models if
  the blocking is done by time slices.
...
- Allow for fitting uncorrelated random slopes and intercepts with the
  form `(1 + slope_var || group)`, making sure that results are
  displayed correctly.

- The correctly transformed correlation between a random slope and
  intercept is now displayed by
  [`print()`](https://rdrr.io/r/base/print.html) and
  [`tidy()`](https://generics.r-lib.org/reference/tidy.html).

- Use [`tryCatch()`](https://rdrr.io/r/base/conditions.html) on Newton
  loops in optimization to abort those steps instead of producing an
  error.

- Allow for missing factor levels when predicting with time-varying
  factor predictors.
  [\#533](https://github.com/sdmTMB/sdmTMB/issues/533)

- Add more useful indicators in
  [`sanity()`](https://sdmTMB.github.io/sdmTMB/reference/sanity.md) by
  identifying the fixed effects.
...
- Add experimental Restricted Spatial Regression (RSR) fixed-effect
  ...
  (Hanks et al. 2015, Diaz and Thorson 2025). Opt in via
  `control = sdmTMBcontrol(get_rsr = TRUE)` and access via
...
- Add `derived_link` in
  ...
  to allow override of inverse link. Useful for fitting Bernoulli data
  with a cloglog link but then predicting as an expectation from a count
  from a thinned count process.
  [\#339](https://github.com/sdmTMB/sdmTMB/issues/339)

- Add `allow_new_levels` in
  [`predict.sdmTMB()`](https://sdmTMB.github.io/sdmTMB/reference/predict.sdmTMB.md)
  for predicting with new levels in random intercepts or slopes. New
  levels get the population value.
  [\#196](https://github.com/sdmTMB/sdmTMB/issues/196)
  [\#480](https://github.com/sdmTMB/sdmTMB/issues/480)

- Add
  [`plot_diffusion_kernel()`](https://sdmTMB.github.io/sdmTMB/reference/covariate_diffusion_plots.md)
  for plotting covariate-diffusion impulse responses and
  [`plot_diffused_covariate()`](https://sdmTMB.github.io/sdmTMB/reference/covariate_diffusion_plots.md)
  for plotting original and estimated diffused covariate fields on the
  fitted mesh. [\#507](https://github.com/sdmTMB/sdmTMB/issues/507)

- Add experimental support for CAR/SAR areal/lattice models.

- Add covariate-diffusion article/vignette.
  [\#507](https://github.com/sdmTMB/sdmTMB/issues/507)
...
- Update
  [`get_index()`](https://sdmTMB.github.io/sdmTMB/reference/get_index.md)
  so index totals are reported on a log-total scale after integrating
  response-scale predictions. This preserves existing behaviour for
  log-link abundance models and allows binomial/beta-binomial proportion
  models, including `logit` and `cloglog` links, to use `area` as an
  explicit standardization multiplier (e.g., hooks per longline set) for
  expected-count indices.

- Fix `se_natural` column in
  [`get_index()`](https://sdmTMB.github.io/sdmTMB/reference/get_index.md)
  output. [\#523](https://github.com/sdmTMB/sdmTMB/issues/523) Thanks to
  [@CataRoman](https://github.com/CataRoman) and
  [@gavinfay](https://github.com/gavinfay).

- Fix [`residuals()`](https://rdrr.io/r/stats/residuals.html) for
  Poisson-link delta models so residuals use the correct inverse link
  for encounter and positive-component mechanism(including offsets),
  avoiding spurious `NaN`/`Inf` values in the first linear predictor
  residuals. For most scenarios, using simulation-based residuals from
  [`dharma_residuals()`](https://sdmTMB.github.io/sdmTMB/reference/dharma_residuals.md)
  for delta models for the combined residuals is still most straight
  forward. [\#512](https://github.com/sdmTMB/sdmTMB/issues/512)

- Fix [`predict()`](https://rdrr.io/r/stats/predict.html) with `newdata`
  containing `NA` in the response column for models with random effects.
  Predictions now work as expected when the response column is present
  but unused for prediction.
