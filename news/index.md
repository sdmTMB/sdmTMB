...
- Fix [`predict()`](https://rdrr.io/r/stats/predict.html) with
  ...
  on combined predictions when `model = NA` (default). Previously,
  standard errors were only calculated for the first component (`est1`)
  instead of the combined estimate (`est`).
