...
test_that("a saved RTMB fit keeps its backend in a fresh R session", {
  skip_on_cran()
  skip_on_ci() # spawns a fresh Rscript subprocess; unreliable across CI runners
  set.seed(4)
  d <- data.frame(x = seq(-1, 1, length.out = 20L))
  d$y <- 0.2 + 0.5 * d$x + rnorm(nrow(d), sd = 0.5)
  fit <- sdmTMB(y ~ x, data = d, spatial = "off",
  ...
  saveRDS(fit, saved)
  # Load the package under test: the source tree under devtools, otherwise
  # the installed copy (e.g., in R CMD check).
  path <- getNamespaceInfo("sdmTMB", "path")
  load <- if (exists(".__DEVTOOLS__", asNamespace("sdmTMB"))) {
    sprintf("pkgload::load_all(%s, compile = FALSE, quiet = TRUE)",
      deparse(path))
  } else {
    "library(sdmTMB)"
  }
  ...
  writeLines(c(
    ...
    load,
    "options(sdmTMB.backend = 'tmb')",
    sprintf("fit <- readRDS(%s)", deparse(saved)),
    "updated <- update(fit, y ~ 1)",
    "est <- predict(fit, newdata = fit$data[1:3, ], se_fit = TRUE)$est_se",
    sprintf("saveRDS(list(backend = updated$backend, est_se = est), %s)",
      deparse(result))
  ), script)
  status <- system2(file.path(R.home("bin"), "Rscript"),
    c("--vanilla", shQuote(script)), stdout = FALSE, stderr = FALSE)
  expect_identical(status, 0L)
  out <- readRDS(result)
  expect_identical(out$backend, "rtmb")
  expect_equal(out$est_se,
    suppressMessages(predict(fit, newdata = d[1:3, ], se_fit = TRUE))$est_se)
})
