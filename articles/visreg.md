...
visreg plots use ggplot2, so we can customize them with ggplot2
functions:

``` r

g <- visreg(fit, xvar = "depth_scaled")
g + xlab("Depth scaled")
```
...
head(d$fit)
#>   depth_scaled fyear density visreg_fit visreg_lwr visreg_upr
...
#> 6    -2.652235  2011       0   2.116734   1.180157   3.053310
g <- ggplot(d$fit, aes(x = depth_scaled, y = visreg_fit)) +
  geom_line() +
  geom_ribbon(aes(ymin = visreg_lwr, ymax = visreg_upr), alpha = 0.5)

# residuals are in d$res
head(d$res)
#>   depth_scaled fyear density visreg_res visreg_pos
...
g + geom_point(aes(y = visreg_res), data = d$res, size = 1, alpha = 0.4)
```
...
produces a ggplot2 raster and contour plot. To produce a static 3D
perspective plot, save the calculations and pass them to
[`persp()`](https://rdrr.io/r/graphics/persp.html):

``` r

v2d <- visreg2d(fit, xvar = "fyear", yvar = "depth_scaled", plot = FALSE)
persp(v2d)
```
...
![](visreg_files/figure-html/unnamed-chunk-11-2.png)
