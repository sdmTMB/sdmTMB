...
#> Chain 1: 
#> Chain 1: Gradient evaluation took 0.000962 seconds
#> Chain 1: 1000 transitions using 10 leapfrog steps per transition would take 9.62 seconds.
...
#> Chain 1:                4.691 seconds (Sampling)
#> Chain 1:                11.473 seconds (Total)
#> Chain 1:
...
E.g., let’s extract the DHARMa residuals from a model and make a QQ plot
for each year:
...
$`r_1 \sim \operatorname{N}(0, 1)`$, when if calculated correctly we
know they do come from $`\operatorname{N}(0, 1)`$
...
#> Chain 1: 
#> Chain 1:  Elapsed Time: 30.811 seconds (Warm-up)
#> Chain 1:                28.388 seconds (Sampling)
#> Chain 1:                59.199 seconds (Total)
#> Chain 1:
