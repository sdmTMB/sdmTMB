...
from an NB2 negative binomial observation model with a spatial random
field, an intercept, and one predictor named `a1` that has a linear
...
second model will match the simulated data. The third model is missing
the ‘a1’ predictor. We’ll use a penalized complexity (PC) prior on the
...
the spatial random-field standard deviation (SD) relative to the truth.
The model missing the `a1` predictor does so to a lesser degree.

### Analytical randomized-quantile residuals

Here are randomized quantile residuals with fixed effects fixed at their
MLEs (maximum likelihood estimates) and random effects taken from a
single sample of their approximate distribution (more details below):
...
residuals with the relevant distribution function so that they should
follow a uniform(0, 1) distribution under the model, and then optionally
transform those values so that they should follow a normal(0, 1)
distribution. You can see the source code at
<https://github.com/sdmTMB/sdmTMB/blob/master/R/residuals.R>

We can see here that the Poisson model likely has problems in the tails.

### MCMC-based randomized-quantile residuals

The above approach assumes that a draw of the random effects can be
approximated by a multivariate normal distribution. If we want to relax
that assumption, we can sample the random effects with MCMC while
holding the fixed effects at their MLEs. We do this with the
...
#> Chain 1: 
#> Chain 1: Gradient evaluation took 0.000985 seconds
#> Chain 1: 1000 transitions using 10 leapfrog steps per transition would take 9.85 seconds.
...
#> Chain 1: 
#> Chain 1:  Elapsed Time: 6.839 seconds (Warm-up)
#> Chain 1:                4.759 seconds (Sampling)
...
These return a matrix where each row represents an observation and each
...
There are clearly too few zeros in the data simulated from the Poisson
model, whereas the NB2 model seems reasonable.
...
relative to the Poisson model. Note the values clumping near 1.0 on the
observed axis and deviating downwards towards 0.0 observed. This is
indicative of too many zeros and variance increasing too rapidly with
the mean for the Poisson distribution, resulting in some large outlying
values.

Let’s try the correct model:
...
Let’s try plotting the residuals against the missing predictor:
...
We can see a trend in the residuals against `a1` because we omitted it
from the model.
...
## Extracting DHARMa residuals

We can extract the DHARMa residuals if we want to plot them ourselves.
For example, let’s extract the DHARMa residuals from a model and make a
QQ plot for each year:
...
# make sure to set return_DHARMa = TRUE:
dh <- simulate(fit_pcod, nsim = 300, type = "mle-mvn") |> 
  dharma_residuals(fit_pcod, return_DHARMa = TRUE)

# add the DHARMa scaled residuals to the fitted data:
pcod_dat$scaled_residuals <- dh$scaledResiduals

# a small function to calculate the quantile-quantile values:
make_qq <- function(x) {
  u <- x$scaled_residuals
  n <- length(u)
  m <- seq_len(n) / (n + 1)
  z <- qqplot(m, u, plot.it = FALSE)
}

# apply our function to each year:
out <- group_by(pcod_dat, year) |> 
  group_split() |> 
  purrr::map_dfr(make_qq, .id = "year")
...
![](residual-checking_files/figure-html/unnamed-chunk-14-1.png)
...
in an unexported function, `oneSamplePosterior()`, within TMB. Thygesen
et al. (2017) also describe it in the context of one-sample MCMC
residuals.
...
$`r_1 \sim \operatorname{N}(0, 1)`$, even though if the residuals are
calculated correctly we know they do come from
$`\operatorname{N}(0, 1)`$.

If instead we return to a single sample from the assumed MVN
random-effect distribution, we get the “correct” residuals:
...
$`r_2 \sim \operatorname{N}(0, 1)`$.

We could also sample that draw of random effects using MCMC (with the
fixed effects still held at their MLEs), which relaxes our assumptions,
but is much more time-intensive for large models.
...
#> SAMPLING FOR MODEL 'tmb_generic' NOW (CHAIN 1).
#> Chain 1: 
#> Chain 1: Gradient evaluation took 0.002211 seconds
#> Chain 1: 1000 transitions using 10 leapfrog steps per transition would take 22.11 seconds.
#> Chain 1: Adjust your expectations accordingly!
#> Chain 1: 
#> Chain 1: 
...
#> Chain 1: 
#> Chain 1:  Elapsed Time: 30.698 seconds (Warm-up)
#> Chain 1:                28.215 seconds (Sampling)
#> Chain 1:                58.913 seconds (Total)
#> Chain 1:
...
#> 
#> data:  r3
#> D = 0.024984, p-value = 0.5604
#> alternative hypothesis: two-sided
```

Here that gets us something similar, and we would (correctly) fail to
reject the hypothesis that $`r_3 \sim \operatorname{N}(0, 1)`$.
...
![](residual-checking_files/figure-html/unnamed-chunk-21-1.png)

We get back to something with the wrong reference distribution for
...
Let’s simulate data again, but with large observation error (`phi` here,
which is the Gaussian error SD in this case) and smaller random-field
variances (`sigma_E` and `sigma_O`):
...
![](residual-checking_files/figure-html/unnamed-chunk-24-1.png)
...
Now, it does not really matter because the “incorrect” random-effect
distribution is swamped by the observation-error contribution to the
distribution. Technically, the first set is “wrong” and the second set
is “right”, but functionally we would come to a similar conclusion in
this case.
...
to use is largely a matter of preference, but it is important to
appreciate how this changes the appearance of the expected residuals.
...
![](residual-checking_files/figure-html/unnamed-chunk-28-2.png)
...
![](residual-checking_files/figure-html/unnamed-chunk-29-1.png)
...
The normal(0, 1) residuals put more emphasis on the tails. This is both
good and bad: it is easier to examine tail behaviour, but these
residuals can often look “off” even when the model is fine (as in this
example) because observations in the tails of the distribution are, by
definition, rarely observed.
...
Either is valid, but you do need to adjust your expectations
accordingly. For example, poor tail behaviour may look like a minor
issue with uniform residuals; conversely, the tails of normal(0, 1)
residuals are unlikely ever to look “perfect” without large sample
sizes.
...
![](residual-checking_files/figure-html/unnamed-chunk-31-1.png)
...
![](residual-checking_files/figure-html/unnamed-chunk-31-2.png)

This works for any distribution as long as we can define the
...
take a gamma-distributed variable and turn it into a uniformly
distributed variable.

Now we need to do the same with the Poisson equivalent:
[`ppois()`](https://rdrr.io/r/stats/Poisson.html). The `ppois(y)`
function gives us the cumulative probability up to value `y`. Say we
have a Poisson variable with a mean (lambda) of 5 and we have an
...
[`ppois()`](https://rdrr.io/r/stats/Poisson.html) to the observed
values, we end up with discrete cumulative probabilities that are not
very useful for comparing against a continuous uniform distribution.
...
Instead, we need to get the cumulative probability up to the value below
the one we observed and “fill in” the values up to the observed value
with the desired uniformly distributed samples. First, get the
cumulative probability up to the value below and up to the observed
value:
...
![](residual-checking_files/figure-html/unnamed-chunk-36-2.png)
...
![](residual-checking_files/figure-html/unnamed-chunk-38-1.png)
