...
and spatiotemporal settings, including the introduction of the MSDK and
RMSDK terms.
...
#> 4 RMSDK[x1]       0.530    0.465    0.0951      2.96
```

Here `kappaS_nl[x1]` is the estimated spatial diffusion scale parameter
for `x1` (larger values imply less smoothing).

The output also reports `MSDK[x1]` and `RMSDK[x1]`, the implied
mean-squared diffusion-kernel scale (`4 / kappaS_nl[x1]^2`) and its
square root (`2 / kappaS_nl[x1]`). We describe these in the next
section. By default, `log_kappaS_nl` is bounded so that RMSDK stays
...
triggers a warning. Supply `lower` or `upper` for `log_kappaS_nl` in
[`sdmTMBcontrol()`](https://sdmTMB.github.io/sdmTMB/reference/sdmTMBcontrol.md)
to change these.

#### SR, MSDK, RMSDK, MSD

Thorson *et al.* (2026) defines 3 quantities: SR, MSD, and MSDK (and
MSDK’s square root: RMSDK).

SR is the “squared resolution”. When the covariate gets projected to the
mesh vertices there’s a degree of smoothing. That’s what SR is.

MSDK is the “mean squared diffusion kernel”. It describes the amount of
diffusion incorporating both the spatial and any estimate temporal
diffusion.

MSD is the “mean squared diplacement”: It is the realized diffusion at a
given time step. Effectively that means it’s SR + one MSDK per time
step.

RMSDK is the square root of MSDK. This puts it back in distance units.

Currently, sdmTMB reports MSDK and RMSDK. SR and MSD require defining a
fine-resolution grid. We may add these quantities in the future if there
is interest.

Users using these MSDK and RMSDK metrics should consult and cite Thorson
*et al.* (2026).
...
components: `~ diffusion(x1) + time_lag(x1)`. Because both wrappers
refer to `x1`, they select parts of one stationary joint operator and
produce one transformed predictor and one coefficient. Wrappers applied
to different covariates, such as `~ diffusion(x1) + time_lag(x2)`,
produce separate predictors and coefficients.
...
#> ML criterion at convergence: -4054.339
#> 
#> See ?tidy.sdmTMB to extract these values as a data frame.
...
#>   term          estimate std.error conf.low conf.high
#>   <chr>            <dbl>     <dbl>    <dbl>     <dbl>
#> 1 range           0.279    0.0503    0.196      0.398
#> 2 phi             0.0998   0.00103   0.0978     0.102
#> 3 sigma_O         0.173    0.0174    0.142      0.211
...
#> 8 RMSDK[x1]       0.379    0.00447   0.371      0.388
```

In the random-effect parameters, `kappaS_nl[x1]` controls spatial
diffusion and `kappaT_nl[x1]` controls temporal persistence in the joint
operator. The transformed predictor is named `nl_diffusion_time_lag_x1`
because the two wrappers are grouped by covariate. Users applying the
...
``` math
(\mathbf{M}_0 + \kappa_S^{-2}\mathbf{M}_1)\mathbf{z}_t = \mathbf{M}_0\mathbf{x}_t.
```

For a temporal lag only, the recursion is

``` math
z_t = \frac{x_t + \kappa_T z_{t-1}}{1 + \kappa_T}.
```

For the joint space–time operator,

``` math
\left[(1 + \kappa_T)\mathbf{M}_0 + \kappa_S^{-2}\mathbf{M}_1\right]\mathbf{z}_t
= \mathbf{M}_0\mathbf{x}_t + \kappa_T\mathbf{M}_0\mathbf{z}_{t-1}.
```

With $`\rho_T = \kappa_T/(1 + \kappa_T)`$, the temporal-only recursion
is $`z_t = (1 - \rho_T)x_t + \rho_T z_{t-1}`$. The temporal parameter is
estimated as $`\log \kappa_T`$, which equals
$`\operatorname{logit} \rho_T`$, so $`0 < \rho_T < 1`$ without bounds.
As $`\kappa_T \to 0`$, the joint operator reduces to spatial diffusion;
to fit no temporal lag, drop `time_lag()`. The implementation
corresponds to `kappaST = 0` in the formulation described by Thorson *et
al.* (2026).

Both recursions need a transformed state $`\mathbf{z}_0`$ before the
first modeled slice. By default (`time_lag(x, start = "stationary")`),
sdmTMB assumes the covariate held at its first slice $`\mathbf{x}_1`$
beforehand, so $`\mathbf{z}_0`$ is the recursion’s steady state for
$`\mathbf{x}_1`$. That is $`\mathbf{z}_0 = \mathbf{x}_1`$ for a temporal
lag alone, and the spatial diffusion of $`\mathbf{x}_1`$,
$`(\mathbf{M}_0 + \kappa_S^{-2}\mathbf{M}_1)\mathbf{z}_0 = \mathbf{M}_0\mathbf{x}_1`$,
for the joint operator.

Thorson *et al.* (2026) instead starts from
$`\mathbf{z}_0 = \mathbf{0}`$, which you can select with
`time_lag(x, start = "zero")`. With that start, the weights on past
covariate values sum to $`1 - \rho_T^t`$ rather than 1, so early slices
are shrunk towards zero. The fit then depends on where the covariate’s
zero is. For example, adding a constant $`c`$ to $`x`$ shifts $`z_t`$ by
$`c(1 - \rho_T^t)`$ instead of by $`c`$. A single intercept cannot
absorb that, so the fit can change with the covariate’s units. A full
set of time effects (e.g., `0 + factor(year)`) absorbs it, because the
shift is the same everywhere in space within each slice. With the
stationary start, adding $`c`$ to $`x`$ adds exactly $`c`$ to $`z`$.
...
We will plot the jointly transformed covariate on the non-local data
grid locations:
...
We can inspect the spatial and temporal parts of the fitted operator, or
the full joint effect, as either the impulse-response kernel or the
transformed covariate field. By default, the diffusion kernel is shown
...
We could also have plotted the jointly transformed covariate at the
...
- Use [`predict()`](https://rdrr.io/r/stats/predict.html) and the `nl_*`
  columns for the modeled effect. Columns such as `nl_diffusion_x1`,
  `nl_time_lag_x1`, and `nl_diffusion_time_lag_x1` are the transformed
  covariate values that enter the linear predictor at your data or
  `newdata` locations. The last name is used when both wrappers are
  applied to `x1`. For `time_lag()` these apply the stationary temporal
  recursion at each vertex, for `diffusion()` they give the spatially
  smoothed field, and for the grouped space–time case they give the
  jointly transformed field. This is what you want for reporting,
  mapping the estimated effect, correlating against a known truth, or
  any downstream calculation. One column is returned per fitted
  transformed operator/design column.
...
- Use
  ...
  space-and-time response). This diagnostic uses the same joint operator
  as the `nl_diffusion_time_lag_x1` prediction column. Both plot on the
  mesh vertices by default, or at supplied `newdata` coordinates as
  points or a raster.
