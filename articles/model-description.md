...
details are available in the sdmTMB paper (Anderson *et al.* 2025),
...
- $`\mathrm{Var}[y]`$ to define the expected variance of the variable
  $`y`$,

- a $`^*`$ superscript represents interpolated or projected values as
  opposed to values at knot locations (e.g., $`\boldsymbol{\omega}`$
  vs. $`\boldsymbol{\omega}^*`$), and
...
- $`\boldsymbol{\beta}`$ represents a vector of fixed-effect
  coefficients;
- $`O_{\mathbf{s},t}`$ represents an offset: a covariate (usually
  log-transformed) with a coefficient fixed at one that enters on the
  linear predictor scale (before the inverse link);
- $`\mathbf{Z}_{g}`$ represents the random-effect design row(s) for
  group $`g`$ (a subset of the matrix $`\mathbf{Z}`$) corresponding to
  $`\mathbf{b}_{g}`$; these random effects enter on the linear predictor
  scale; the scalar special case is a random intercept
  $`\alpha_{g}\sim \mathrm{N}(0,\sigma_\alpha^2)`$, and more generally
  random slopes and intercepts are vector-valued with full covariance as
  described below;
...
- $`\boldsymbol{\zeta}_{\mathbf{s}}`$ represents a vector of spatially
  varying coefficients (random fields), where each coefficient
  $`\zeta_{l,\mathbf{s}} \sim \mathrm{MVN}(\boldsymbol{0},\boldsymbol{\Sigma}_{\zeta,l})`$;
- $`\omega_{\mathbf{s}}`$ represents a spatial component (a Gaussian
  Markov random field, GMRF),
  $`\omega_{\mathbf{s}} \sim \mathrm{MVN}(\boldsymbol{0},\boldsymbol{\Sigma}_\omega)`$
  with Matérn covariance built from the mesh; and
- $`\epsilon_{\mathbf{s},t}`$ represents a spatiotemporal component (a
  GMRF),
  $`\epsilon_{\mathbf{s},t} \sim \mathrm{MVN}(\boldsymbol{0},\boldsymbol{\Sigma}_{\epsilon})`$
...
so that for $`x < b_{1}`$, $`s(x) = x \cdot b_{0}`$, and for
$`x > b_{1}`$, $`s(x) = b_{1} \cdot b_{0}`$. In both cases, $`b_{0}`$
represents the slope of the function up to a threshold, and the product
$`b_{1} \cdot b_{0}`$ represents the value at the asymptote. No
constraints are placed on parameters $`b_{0}`$ or $`b_{1}`$.
...
parameter (controlling the height of the response curve; unconstrained),
$`\tau`$ is an intercept, $`s50`$ is a parameter controlling the point
at which the function reaches 50% of the maximum ($`\psi`$), and $`s95`$
is a parameter controlling the point at which the function reaches 95%
of the maximum. The parameter $`s50`$ is unconstrained but $`s95`$ is
...
\end{aligned}
```
The marginal standard deviation of $`\boldsymbol{\omega}`$ is indicated
by `Spatial SD` in the printed model output or as `sigma_O` in the
output of `sdmTMB::tidy(fit, "ran_pars")`. The ‘O’ is for ‘omega’
($`\omega`$).
...
the representation in VAST (Thorson 2019). The marginal standard
deviation of $`\boldsymbol{\epsilon}_t`$ is indicated by
`Spatiotemporal SD` in the printed model output or as `sigma_E` in the
output of `sdmTMB::tidy(fit, "ran_pars")`. The ‘E’ is for ‘epsilon’
($`\epsilon`$).
...
random fields. The
$`\rho \boldsymbol{\delta}_{t-1} + \sqrt{1 - \rho^2}`$ term scales the
...
which is unconstrained. The parameter `ar1_phi` is transformed to
$`\rho`$ with
...
In this case, the first time-step value is treated as an unconstrained
parameter (equivalent to an improper flat prior). That is, the same
variable should not appear in the fixed-effects formula since the
initial value is estimated as part of the time-varying formula. The
formula `time_varying = ~ 1` implicitly represents a time-varying
intercept (assuming the `time` argument has been supplied) and, in this
case, the intercept should be omitted from the main-effects formula
(e.g., `formula = y ~ 0 + ...` or `formula = y ~ -1 + ...`).
...
steps. The first time step is given a mean-zero prior.
...
special case $`(1|g)`$ corresponds to $`n=1`$ with
$`\alpha_{g} \sim \mathrm{Normal}(0, \sigma_{\alpha}^2)`$.
...
$`\boldsymbol{\Sigma}_{g}`$ per grouping factor.
...
Distributed lag terms (Lindmark *et al.* 2025; Thorson *et al.* 2026)
can be added with the `nonlocal_formula` argument to
[`sdmTMB()`](https://sdmTMB.github.io/sdmTMB/reference/sdmTMB.md) as a
one-sided formula using `diffusion()` and/or `time_lag()` wrappers. For
example: `~ diffusion(x) + time_lag(x)`. Conceptually, these nonlocal
terms let the effect of a covariate be distributed over nearby locations
and/or previous time steps, rather than acting only locally and
instantaneously. This can be useful when ecological responses are
delayed, transported, or accumulated through space and time. Users
should consult (and cite) Lindmark *et al.* (2025) (spatial) and Thorson
*et al.* (2026) (extension to time) for details on the model formulation
and interpretation.

Wrappers are grouped by covariate. If the same covariate appears in both
wrappers, as in `~ diffusion(x) + time_lag(x)`, they select parts of one
joint operator and produce one transformed predictor and one
coefficient. If the covariates differ, as in
`~ diffusion(x1) + time_lag(x2)`, the model has two separately
transformed predictors and two coefficients. Covariates are ordered by
their first appearance in the formula.

These diffused terms enter the linear predictor as transformed
covariates with estimated coefficients:

``` math
\mu_{\mathbf{s},t} = f^{-1} \left(\ldots + \sum_{r=1}^{R} \beta^{\mathrm{dl}}_{r}\,\tilde{x}_{r,\mathbf{s},t} + \ldots \right),
```

where each transformed covariate $`\tilde{x}_{r,\mathbf{s},t}`$ is
constructed from the corresponding observed covariate using one of the
components below.

For `diffusion(x)`, each time slice is spatially smoothed:

``` math
\left(\mathbf{M}_0 + \kappa_{S}^{-2}\mathbf{M}_1\right)\tilde{\mathbf{x}}_{t} = \mathbf{M}_0\mathbf{x}_{t},
```

where $`\mathbf{x}_t`$ is the covariate evaluated at mesh vertices at
time $`t`$, and $`\mathbf{M}_0`$ and $`\mathbf{M}_1`$ are SPDE mass and
stiffness matrices.

For `time_lag(x)`, the transformed covariate follows the stationary
temporal recursion at each vertex:

``` math
\tilde{x}_{\mathbf{s},1} = \frac{x_{\mathbf{s},1}}{1 + \kappa_T}, \qquad
\tilde{x}_{\mathbf{s},t} = \frac{x_{\mathbf{s},t} + \kappa_T\tilde{x}_{\mathbf{s},t-1}}{1 + \kappa_T} \quad (t > 1).
```

This initialization assumes a zero transformed state immediately before
the first modeled time slice. Equivalently, with
$`\rho_T = \kappa_T/(1 + \kappa_T)`$, the recursion is
$`\tilde{x}_{\mathbf{s},1} = (1 - \rho_T)x_{\mathbf{s},1}`$ and
$`\tilde{x}_{\mathbf{s},t} = (1 - \rho_T)x_{\mathbf{s},t} + \rho_T\tilde{x}_{\mathbf{s},t-1}`$.
Fitting constrains $`\kappa_T \geq 0`$, so $`0 \leq \rho_T < 1`$.
Lag-scale parameters are estimated separately for each covariate
included in `nonlocal_formula`. `time_lag()` terms require a `time`
argument in
[`sdmTMB()`](https://sdmTMB.github.io/sdmTMB/reference/sdmTMB.md).

For a covariate with both wrappers, the stationary joint operator is

``` math
\left[(1 + \kappa_T)\mathbf{M}_0 + \kappa_S^{-2}\mathbf{M}_1\right]\tilde{\mathbf{x}}_t
= \mathbf{M}_0\mathbf{x}_t + \kappa_T\mathbf{M}_0\tilde{\mathbf{x}}_{t-1},
```

where the final term is omitted for the first time slice. Thus spatial
smoothing and temporal propagation act on the same evolving field, which
enters the linear predictor once. The implementation corresponds to
`kappaST = 0` in the formulation described by Thorson *et al.* (2026).
At `kappaT = 0`, the joint operator reduces exactly to the spatial-only
operator.

For spatial-only terms, the model output reports `MSDK[...]` and
`RMSDK[...]`, the mean-squared diffusion-kernel scale and its square
root:

``` math
\mathrm{MSDK} = 4\kappa_S^{-2}, \qquad
\mathrm{RMSDK} = \sqrt{\mathrm{MSDK}}.
```

For the stationary joint operator, with
$`\rho_T = \kappa_T/(1 + \kappa_T)`$,

``` math
\mathrm{MSDK} = 4\kappa_S^{-2}(1 - \rho_T)
= \frac{4\kappa_S^{-2}}{1 + \kappa_T}, \qquad
\mathrm{RMSDK} = \sqrt{\mathrm{MSDK}}.
```

Temporal-only terms do not report MSDK or RMSDK. MSDK is the approximate
slope of normalized point-response spatial MSD against time index in the
stationary `kappaST = 0` formulation described by Thorson *et al.*
(2026); it is not time-specific total MSD. The SR term in the
relationship
$`\mathrm{MSD}(t^*) \approx \mathrm{SR} + t^*\mathrm{MSDK}`$ depends on
the FEM basis, impulse location, projection grid, cell-area weights, and
boundaries, so sdmTMB does not report it.
...
matrix directly, so neighbor influence depends on the original edge
weights (and, if unweighted, on the number of neighbors) rather than
...
$`D_{ii} = \sum_j W_{ij}`$ and $`\alpha_{\mathrm{CAR}} \in (0, 1)`$ is
reported as `alpha_car`. CAR requires a symmetric adjacency matrix.
...
the variance is \$ \[y\] = N (1 - ), , \$ which exceeds the Binomial
variance unless $`\phi \to \infty`$. Available links are logit and
...
$`\mathrm{Var}[y] = \mu + \mu / \phi`$(Hilbe 2011). Internally, the
...
where $`\mu_1`$ is the mean of the first (smaller component) of the
distribution, $`\mu_2`$ is the mean of the larger component, and $`p`$
...
(2025), with parameters mean $`\mu`$, scale $`\phi`$ (standard deviation
on the log scale), and a shape parameter $`Q^{\mathrm{gg}}`$ (reported
as `Generalized gamma Q`).

Here, $`\mu`$ is the mean on the data scale, $`\phi`$ is the log-scale
standard deviation, and $`Q^{\mathrm{gg}}`$ controls the shape
...
$`\mathrm{logit}^{-1} (\texttt{thetaf}) + 1`$ to constrain it between 1
and 2 and is estimated as an unconstrained variable.
...
$`\mathbb{E}[y] = (1-p) \cdot \mu_{1} + p \cdot \mu_{2}`$. The log-scale
...
where $`\nu`$, the degrees of freedom (`df`), is estimated by default
and can optionally be fixed by the user. Lower values of $`\nu`$ result
in heavier tails compared to the Gaussian distribution. Above
approximately `df = 20`, the distribution becomes very similar to the
Gaussian. The Student-t distribution with a low degrees of freedom
(e.g., $`\nu \le 7`$) can be helpful for modelling data that would
otherwise be suitable for Gaussian but needs an approach that is robust
to outliers (e.g., Anderson *et al.* 2017).
...
The Matérn defines the covariance $`\Phi \left( s_j, s_k \right)`$
between spatial locations $`s_j`$ and $`s_k`$ as

``` math
\Phi\left( s_j,s_k \right) = \tau^2 \left[ 2^{\nu - 1}\Gamma(\nu) \right]^{-1} (\kappa d_{jk})^\nu K_\nu \left( \kappa d_{jk} \right),
```

where $`\tau^2`$ is a scale/precision factor (higher $`\tau`$ means
lower marginal variance), $`\nu`$ controls the smoothness, $`\Gamma`$
represents the Gamma function, $`d_{jk}`$ represents the distance
between locations $`s_j`$ and $`s_k`$, $`K_\nu`$ represents the modified
Bessel function of the second kind, and $`\kappa`$ represents the
decorrelation rate. The parameter $`\nu`$ is set to 1 because the SPDE
approach that gives us sparse matrices (and therefore major
computational speedups) requires $`\alpha = \nu + d/2`$ to be an integer
when working in 2D (so $`\nu = 1`$) (Lindgren *et al.* 2011).
Internally, the parameters $`\kappa`$ and $`\tau`$ are converted to
range and marginal standard deviation $`\sigma`$ as
$`\textrm{range} = \sqrt{8} / \kappa`$ and
...
projection matrix $`\mathbf{A}`$ that linearly interpolates within mesh
triangles from the knot locations to the locations of the observed or
...
and a column for each knot. Three non-zero elements on each row define
the weights of the three neighbouring knot locations for location
$`\mathbf{s}`$. The same interpolation happens for any spatiotemporal
random fields
...
al.* 2015), and we adopt a 2-parameter rotation matrix $`\mathbf{H}`$.
The elements of $`\mathbf{H}`$ are defined by the parameter vector
$`\boldsymbol{x}`$ so that $`H_{1,1} = x_{1}`$,
$`H_{1,2} = H_{2,1} = x_{2}`$ and $`H_{2,2} = (1 + x_{2}^2) / x_{1}`$.
...
with respect to fixed-effect parameters is not small enough yet), and
the algorithm may need to be run multiple times. In the
[`sdmTMB::sdmTMBcontrol()`](https://sdmTMB.github.io/sdmTMB/reference/sdmTMBcontrol.md)
...
| $`\boldsymbol{\Sigma}_{g}`$ | `re_cov_pars` | Random effect covariance per grouping factor (Cholesky SD + correlation params) |
