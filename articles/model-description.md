...
- bold symbols for vectors or matrices (e.g., $`\boldsymbol{\omega}`$ is
  ...
  $`\boldsymbol{\omega}`$ at point in space $`\mathbf{s}`$), with
...
unconstrained), $`\tau`$ is an intercept, $`s50`$ is a parameter
controlling the point at which the function reaches 50% of the maximum
($`\psi`$), and $`s95`$ is a parameter controlling the point at which
the function reaches 95% of the maximum. The parameter $`s50`$ is
unconstrained but $`s95`$ is constrained to be larger than $`s50`$.
...
$`\mathbf{X}^{\mathrm{svc}}_{\mathbf{s}, t}`$ would represent a
prediction matrix that is constant spatially for a given time $`t`$ as
...
Covariate diffusion terms (Lindmark *et al.* 2025; Thorson *et al.*
2026) can be added with the `covariate_diffusion` argument to
[`sdmTMB()`](https://sdmTMB.github.io/sdmTMB/reference/sdmTMB.md) as a
one-sided formula using `space()` and/or
[`time()`](https://rdrr.io/r/stats/time.html) wrappers. For example:
`~ space(x) + time(x)`. Conceptually, covariate diffusion lets the
effect of a covariate be distributed over nearby locations and/or
previous time steps, rather than acting only locally and
...
For `space(x)`, each time slice is spatially smoothed:
...
For `time(x)`, the transformed covariate follows a temporal recursion at
each vertex:
...
included in `covariate_diffusion`.
[`time()`](https://rdrr.io/r/stats/time.html) terms require a `time`
argument in
[`sdmTMB()`](https://sdmTMB.github.io/sdmTMB/reference/sdmTMB.md).
...
overdispersion relative to a Binomial. The implied Beta parameters are
$`\alpha = \mu \phi`$ and $`\beta = (1 - \mu)\phi`$, and the variance is
\$ \[y\] = N (1 - ), , \$ which exceeds the Binomial variance unless
$`\phi \to \infty`$. Available links are logit and cloglog. This family
is useful for overdispersed counts of successes/failures (e.g.,
aggregated Bernoulli data, proportions with extra-binomial variation).
...
the weight of the neighbouring 3 knot locations for location
$`\mathbf{s}`$. The same bilinear interpolation happens for any
spatiotemporal random fields
