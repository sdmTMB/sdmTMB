...
For `time_lag(x)`, the transformed covariate follows a temporal
recursion at each vertex:

``` math
\tilde{x}_{\mathbf{s},1} = x_{\mathbf{s},1}, \qquad
\tilde{x}_{\mathbf{s},t} = x_{\mathbf{s},t} + \kappa_{T}\tilde{x}_{\mathbf{s},t-1} \quad (t > 1).
```
...
For spatial diffusion terms, the model output also reports `MSD[...]`
and `RMSD[...]`, the implied mean squared displacement and root mean
squared displacement. With the current parameterization,
`\mathrm{MSD} = 4/\kappa_S^2` and `\mathrm{RMSD} = 2/\kappa_S`; `RMSD`
is a typical diffusion distance (a blur radius) in coordinate units,
where larger values imply broader smoothing and smaller values imply
more local diffusion.
