...
Cross-validation is one of the best approaches for quantifying model
performance and comparing sdmTMB models with different structures
(unlike AIC, this approach also accounts for uncertainty in random
effects). Arguably the most challenging decision when implementing
...
prediction; these include predicting to new spatial regions
...
Alternatively, the goal of an analysis with sdmTMB may be to evaluate
the predictive accuracy of a model through time (e.g. for a missing
survey year, or for prediction to future years). For retrospective
analyses, all points within a year may be assigned to a fold (or groups
of years to the same fold). In contrast, models that are forward looking
would use Leave Future Out Cross-Validation (LFOCV). In LFOCV, data up
...
(defaulting to 8). The function uses parallelization by default if a
...
modified for specific spatial or temporal applications. Without getting
into the complexities of the `blockCV` or `spatialsample` packages, we
could simply use [`kmeans()`](https://rdrr.io/r/stats/kmeans.html) to
generate spatial clusters, e.g.
...
Or similarly, these clusters could be assigned in time, here with each
year assigned to a unique fold. Note that year is not included as a
factor and spatiotemporal fields are turned off because they cannot be
estimated in missing years.
...
Many measures of predictive accuracy can be used to evaluate model
performance. By default,
[`sdmTMB_cv()`](https://sdmTMB.github.io/sdmTMB/reference/sdmTMB_cv.md)
returns a list that contains the sum of the log-likelihood values for
each left-out fold and the total summed across the left-out folds. This
is analogous to the expected log predictive density (ELPD) in the
Bayesian literature and can be interpreted as a measure of predictive
ability for new observations. These can be accessed as below, and
...
represents the sum of `fold_loglik` across the entire dataset. We can
also calculate derived statistics based on the predictions, here using
Root Mean Square Error (RMSE) and Mean Absolute Error (MAE) as examples.
...
will be fit, but using this function may still be worthwhile to reduce
...
[`sdmTMB_cv()`](https://sdmTMB.github.io/sdmTMB/reference/sdmTMB_cv.md)
to compare two or more models. For example, if we wanted to evaluate
support for a depth effect, we could do 10-fold cross-validation (it is
...
Finally, instead of identifying a single “best” model, we may be
interested in model averaging. In the sdmTMB package, we’ve implemented
the model stacking procedure described by (Yao *et al.* 2018) in the
...
split the data into a 10/90 split (as in the example above), we would
not want to use the second model fit to generate these weights. If we
...
of predictive ability are available. These are applicable to
cross-validation, although we demonstrate them here first in a
non-cross-validation context for simplicity.

A commonly used diagnostic is the AUC (area under the curve), which
quantifies the ability of a model to discriminate between the two
classes; this is computed from the Receiver Operating Characteristic
(ROC) curve, which plots the true positive rate versus the false
positive rate. There are several packages to calculate AUC in R, but
this can be done with the `pROC` package, where inputs are a vector of
0s and 1s (or factor equivalents) in the raw data, and a vector of
estimated probabilities (generated from a call to
...
AUC may be sensitive to class imbalance, however, and alternative
metrics may better reflect predictive skill. Here we highlight an
...
models, including methods for evaluating model performance at multiple
...
and may compound or counteract each other when aggregated, it is useful
