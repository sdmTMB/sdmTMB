...
**This feature is experimental and under development.** It is available only with the RTMB backend, via `sdmTMB(..., preferential = preferential_sampling(...))`, and currently supports single log-link families (Poisson, NB2, Gamma, Tweedie, lognormal) without smoothers.
For these families, the link-scale predictor is the log of the response mean.
...
$$
h_{g,t} = \bX^{*}_{g,t} \bbeta + O^{*}_{g,t} + \bA^{*}_{g} \bomega + \bA^{*}_{g} \beps_t,
$$

where $\bX^{*}_{g,t}$ is the main model's fixed-effect design evaluated on the frame's covariate values, using the fitted terms, factor levels, contrasts, and transformation bases; $\bbeta$ are the main model's coefficients; $O^{*}_{g,t}$ is an offset for the surface (by default 0, i.e., unit exposure); and $\bA^{*}_{g}$ projects the main model's spatial and spatiotemporal fields to the frame locations.
The frame's covariate values define the standardization (e.g., gear set to a reference level).
IID random effects of the main model are excluded from the surface.
