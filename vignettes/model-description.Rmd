...
**This feature is under development and can't be fitted yet.** It will be available only with the RTMB backend, via `sdmTMB(..., preferential = preferential_sampling(...))`.

The planned joint model couples the main observation model to a Bernoulli model for whether each eligible cell $g$ of a sampling frame was sampled in time step $t$.
