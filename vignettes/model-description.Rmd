...
This vignette uses the following notation conventions, which generally follow the guidance in @edwards2019:
...
-   bold symbols for vectors or matrices (e.g., $\bomega$ is a vector and $\omega_{\s}$ is the value of $\bomega$ at a point in space $\s$), with Latin/Roman symbols in non-italics (e.g., $\mathbf{X}$, $\mathbf{Q}$),
...
where $s(x)$ denotes the threshold function, $\psi$ is a scaling parameter (controlling the height of the response curve; unconstrained), $\tau$ is an intercept, $s50$ is a parameter controlling the point at which the function reaches 50% of the maximum ($\psi$), and $s95$ is a parameter controlling the point at which the function reaches 95% of the maximum.
...
where $\epsilon_{\s,t}$ represents random field deviations at point $\s$ and time $t$.
...
First-order autoregressive, AR(1), spatiotemporal random fields (`spatiotemporal = 'ar1'`) add a parameter defining the correlation between random field deviations from one time step to the next.
They are defined as
...
In this case, the first time-step value is treated as an unconstrained parameter (equivalent to an improper flat prior).
That is, the same variable should not appear in the fixed-effects formula since the initial value is estimated as part of the time-varying formula.
The formula `time_varying = ~ 1` implicitly represents a time-varying intercept (assuming the `time` argument has been supplied) and, in this case, the intercept should be omitted from the main-effects formula (e.g., `formula = y ~ 0 + ...` or `formula = y ~ -1 + ...`).
...
Usually, $\bX^{\RM{svc}}_{\s, t}$ represents the corresponding design matrix for the spatially varying coefficient terms, as defined by a one-sided formula supplied to `spatial_varying`.
...
Distributed lag terms [@lindmark2025; @thorson2026] can be added with the `nonlocal_formula` argument to `sdmTMB()` as a one-sided formula using `diffusion()` and/or `time_lag()` wrappers.
For example: `~ diffusion(x) + time_lag(x)`.
Conceptually, these nonlocal terms let the effect of a covariate be distributed over nearby locations and/or previous time steps, rather than acting only locally and instantaneously.
This can be useful when ecological responses are delayed, transported, or accumulated through space and time.
Users should consult (and cite) @lindmark2025 (spatial) and @thorson2026 (extension to time) for details on the model formulation and interpretation.

Wrappers are grouped by covariate. If the same covariate appears in both wrappers, as in `~ diffusion(x) + time_lag(x)`, they select parts of one joint operator and produce one transformed predictor and one coefficient. If the covariates differ, as in `~ diffusion(x1) + time_lag(x2)`, the model has two separately transformed predictors and two coefficients. Covariates are ordered by their first appearance in the formula.

These diffused terms enter the linear predictor as transformed covariates with estimated coefficients:

$$
\mu_{\s,t} = f^{-1} \left(\ldots + \sum_{r=1}^{R} \beta^{\RM{dl}}_{r}\,\tilde{x}_{r,\s,t} + \ldots \right),
$$

where each transformed covariate $\tilde{x}_{r,\s,t}$ is constructed from the corresponding observed covariate using one of the components below.
...
where $\mathbf{x}_t$ is the covariate evaluated at mesh vertices at time $t$, and $\mathbf{M}_0$ and $\mathbf{M}_1$ are SPDE mass and stiffness matrices.

For `time_lag(x)`, the transformed covariate follows the stationary temporal recursion at each vertex:

$$
\tilde{x}_{\s,1} = \frac{x_{\s,1}}{1 + \kappa_T}, \qquad
\tilde{x}_{\s,t} = \frac{x_{\s,t} + \kappa_T\tilde{x}_{\s,t-1}}{1 + \kappa_T} \quad (t > 1).
$$

This initialization assumes a zero transformed state immediately before the first modeled time slice. Equivalently, with $\rho_T = \kappa_T/(1 + \kappa_T)$, the recursion is $\tilde{x}_{\s,1} = (1 - \rho_T)x_{\s,1}$ and $\tilde{x}_{\s,t} = (1 - \rho_T)x_{\s,t} + \rho_T\tilde{x}_{\s,t-1}$. Fitting constrains $\kappa_T \geq 0$, so $0 \leq \rho_T < 1$. Lag-scale parameters are estimated separately for each covariate included in `nonlocal_formula`.
`time_lag()` terms require a `time` argument in `sdmTMB()`.
...
where the final term is omitted for the first time slice. Thus spatial smoothing and temporal propagation act on the same evolving field, which enters the linear predictor once. The implementation corresponds to `kappaST = 0` in the formulation described by @thorson2026. At `kappaT = 0`, the joint operator reduces exactly to the spatial-only operator.

For spatial-only terms, the model output reports `MSDK[...]` and `RMSDK[...]`, the mean-squared diffusion-kernel scale and its square root:
...
For the stationary joint operator, with $\rho_T = \kappa_T/(1 + \kappa_T)$,
...
Temporal-only terms do not report MSDK or RMSDK. MSDK is the approximate slope of normalized point-response spatial MSD against time index in the stationary `kappaST = 0` formulation described by @thorson2026; it is not time-specific total MSD. The SR term in the relationship $\mathrm{MSD}(t^*) \approx \mathrm{SR} + t^*\mathrm{MSDK}$ depends on the FEM basis, impulse location, projection grid, cell-area weights, and boundaries, so sdmTMB does not report it.

## Areal autoregressive models (SAR/CAR)

In addition to continuous-space SPDE random fields, sdmTMB can fit areal spatial autoregressive models with `spatial_model = "sar"` or `spatial_model = "car"`.
These require an areal domain supplied to `mesh` with the help of `make_areal_domain()`.

For both areal options, the latent spatial and spatiotemporal effects are Gaussian Markov random fields with precision defined by the areal weight matrix $\mathbf{W}$:

$$
\bomega \sim \RM{MVNormal}\left(\bs{0}, \sigma_\omega^2 \bQ^{-1}\right), \:
\beps_t \sim \RM{MVNormal}\left(\bs{0}, \sigma_\epsilon^2 \bQ^{-1}\right).
$$

For SAR (`spatial_model = "sar"`), the precision matrix is

$$
\bQ_{\mathrm{SAR}} = \left(\mathbf{I} - \rho_{\mathrm{SAR}}\mathbf{W}\right)^\top
\left(\mathbf{I} - \rho_{\mathrm{SAR}}\mathbf{W}\right),
$$

where $\rho_{\mathrm{SAR}} \in (-1, 1)$ is reported as `rho_sar`.
By default, SAR uses a row-normalized $\mathbf{W}$ (`sar_weight_style = "row"`), with an option to use raw weights.
With `sar_weight_style = "raw"`, SAR uses the unnormalized adjacency/weight matrix directly, so neighbor influence depends on the original edge weights (and, if unweighted, on the number of neighbors) rather than being scaled to sum to 1 within each row.

For CAR (`spatial_model = "car"`), the precision matrix is

$$
\bQ_{\mathrm{CAR}} = \mathbf{D} - \alpha_{\mathrm{CAR}}\mathbf{W},
$$

where $\mathbf{D}$ is diagonal with elements $D_{ii} = \sum_j W_{ij}$ and $\alpha_{\mathrm{CAR}} \in (0, 1)$ is reported as `alpha_car`.
CAR requires a symmetric adjacency matrix.

## Preferential sampling

**This feature is experimental.** It is available only with the RTMB backend, via `sdmTMB(..., preferential = preferential_sampling(...))`, and currently supports single log-link families (Poisson, NB2, Gamma, Tweedie, lognormal) and `delta_gamma()` and `delta_lognormal()` families with their default links (standard or Poisson-link).

The joint model couples the main observation model to a Bernoulli model for whether each eligible cell $g$ of a sampling frame was sampled in time step $t$.
This is useful when sample locations depend on the process being measured (e.g., a fishery that preferentially returns to productive grounds).

The shared surface is the log of standardized expected catch, $h_{g,t}$.
It is the main model evaluated on the sampling frame, as with prediction `newdata`:

$$
\eta^{*}_{g,t} = \bX^{*}_{g,t} \bbeta + f^{*}_{g,t} + \alpha^{*}_{g,t} + O^{*}_{g,t} + \bA^{*}_{g} \bomega + \bA^{*}_{g} \beps_t,
$$

where $\bX^{*}_{g,t}$ is the main model's fixed-effect design evaluated on the frame's covariate values, using the fitted terms, factor levels, contrasts, and transformation bases; $\bbeta$ are the main model's coefficients; $f^{*}_{g,t}$ are the main model's (univariate) smoothers evaluated with their fitted bases; $\alpha^{*}_{g,t}$ are the main model's fitted IID random intercepts at the frame's group levels, included only with `re_form_iid = NULL` (by default they are excluded, i.e., 0); $O^{*}_{g,t}$ is an offset for the surface (by default 0, i.e., unit exposure); and $\bA^{*}_{g}$ projects the main model's spatial and spatiotemporal fields to the frame locations.
The frame's covariate values define the standardization (e.g., gear set to a reference level).

For a log-link family, $h_{g,t} = \eta^{*}_{g,t}$.
For a delta model, $\eta^{*}_{1,g,t}$ and $\eta^{*}_{2,g,t}$ are evaluated with each component's own design, coefficients, and fields, and $h$ is the log of encounter probability times positive mean,

$$
h_{g,t} = \log \left( \mathrm{logit}^{-1} \left( \eta^{*}_{1,g,t} \right) \right) + \eta^{*}_{2,g,t},
$$

not the sum of the two link predictors. As in prediction, the offset enters only the positive component.
For a Poisson-link delta model [@thorson2018poisson], the log expected catch is $h_{g,t} = O^{*}_{g,t} + \eta^{*}_{1,g,t} + \eta^{*}_{2,g,t}$, where here $O^{*}_{g,t}$ is excluded from $\eta^{*}_{1,g,t}$ and $\eta^{*}_{2,g,t}$.
As in the observation model, the offset scales expected catch; `predict()` instead ignores offsets for these models.

Whether each cell was sampled is then modeled as

$$
\begin{aligned}
\Pr(R_{g,t} = 1) &= \mathrm{logit}^{-1}\left(\bZ_{g,t} \boldsymbol{\gamma} + \delta_t + b\, h_{g,t} + (b_t - b) (h_{g,t} - \bar{h}_t) + \xi_{g}\right),\\
\bxi &\sim \RM{MVNormal} \left( \bs{0}, \bSigma_\xi \right),
\end{aligned}
$$

where $R_{g,t}$ is the 0/1 sampling indicator (`NA` indicators contribute no likelihood); $\bZ_{g,t} \boldsymbol{\gamma}$ is a separate sampling regression with its own coefficients (e.g., free annual intercepts); $b$ is the preference coefficient (`b_pref`); and $\xi_g$ is an optional, time-invariant, isotropic SPDE spatial field on the model mesh with its own range and marginal standard deviation (reported as `range_xi` and `sigma_xi`) that enters only the sampling model.

By default the preference coefficient is constant ($b_t = b$) and there is no baseline process ($\delta_t = 0$).
With `coefficient = "iid"`, $b_t = b + u_t$ with $u_t \sim \RM{Normal}(0, \sigma_b^2)$; with `coefficient = "rw"`, $b_1 = b$ and $b_t = b_{t-1} + d_t$ with $d_t \sim \RM{Normal}(0, \sigma_b^2)$ on an equally spaced time grid.
The baseline $\delta_t$ is defined in the same way with `baseline = "iid"` or `"rw"` (with $\delta_1 = 0$ for a random walk), with SD $\sigma_\delta$; it replaces free time-step intercepts in $\bZ_{g,t} \boldsymbol{\gamma}$, whose intercept is then the mean or first-time-step baseline.
Every time step of the main model, including `extra_time`, has a $b_t$ and $\delta_t$.
The deviations $b_t - b$ multiply the target centered on $\bar{h}_t$, the mean over the frame rows of time step $t$ of $\eta^{*}_{g,t}$ without its spatial and spatiotemporal fields (for a delta model, the combined target of the two components without their fields).
They therefore change how strongly sampling concentrates on high expected catch within a time step rather than the time step's overall sampling rate, which the intercepts already determine precisely.
Without centering, the Laplace-approximated marginal likelihood tends to estimate $\sigma_b = 0$ even when the slopes vary.
The fields are left out of $\bar{h}_t$ because their frame average would couple every sampling row to every mesh vertex of its time step.
The deviations are reported as `sigma_b_pref`, `sigma_alpha_pref`, `b_pref_t`, and `alpha_pref_t`.
The shared surface is recomputed inside the joint likelihood, which integrates over the shared and sampling-only latent effects.
The reported log likelihood and AIC are therefore for the catch and sampling data jointly.
...
$$ where $N$ is the number of trials, $\mu$ is the mean success probability, and $\phi$ is a precision parameter that controls overdispersion relative to a Binomial distribution.
...
where $\nu$, the degrees of freedom (`df`), is estimated by default and can optionally be fixed by the user.
...
The values of the spatial variables at the knots are multiplied by a projection matrix $\bA$ that linearly interpolates within mesh triangles from the knot locations to the locations of the observed or predicted data [@lindgren2015]
...
Three non-zero elements on each row define the weights of the three neighbouring knot locations for location $\s$.
The same interpolation happens for any spatiotemporal random fields
...
TMB allows for anisotropy, where spatial covariance may depend on direction ([full details](https://kaskr.github.io/adcomp/namespaceR__inla.html)).
...
With `sdmTMB()` models, the mesh construction occurs in two steps: the user (1) constructs a mesh with a call to `sdmTMB::make_mesh()`, and (2) passes the mesh to `sdmTMBextra::add_barrier_mesh()`.
...
In some cases, a single call to `stats::nlminb()` may not result in convergence (e.g., the maximum gradient of the marginal likelihood with respect to fixed-effect parameters is not small enough yet), and the algorithm may need to be run multiple times.
