...
The `model_index()` formula special creates one SVC covariate from a centered, model-derived time index.
For example, a factor with many levels still creates one SVC field rather than one field per level:

```r
fit <- sdmTMB(
  density ~ 0 + factor(year),
  spatial_varying = ~ 0 + model_index(factor(year)),
  data = dat,
  mesh = mesh,
  time = "year",
...
For model component $m$ and fitted time $t$, define

$$
a_{tm} = \bX^I_{tm}\bbeta_m + \bar{\epsilon}_{tm}, \qquad
\bar{\epsilon}_{tm} = n_s^{-1}\sum_{\s=1}^{n_s}\epsilon_{\s tm}.
$$

$\bX^I$ contains only the columns assigned to the wrapped fixed-effect term and the ordinary fixed-effect intercept, when present.
The intercept is important for standard delta models because the encounter inverse link is nonlinear.
The construction excludes other fixed effects, ordinary spatial fields, SVCs, offsets, smoothers, time-varying coefficients, IID random effects, thresholds, and nonlocal terms.

For a one-part model, let $q_t = \log\{f^{-1}(a_{t1})\}$.
For a standard delta model with component inverse links $f_1^{-1}$ and $f_2^{-1}$,

$$
q_t = \log\{f_1^{-1}(a_{t1}) f_2^{-1}(a_{t2})\}.
$$

Zero-truncated count families use their conditional positive expectation.
For Poisson-link delta models, $q_t = a_{t1} + a_{t2}$.
In every case, the SVC covariate is centered only after combining components:

$$
x_t = q_t - n_t^{-1}\sum_{u=1}^{n_t}q_u,
\qquad
\eta_{\s tm} = \ldots + \zeta_{m,\s}x_t.
$$

The fixed temporal effect and local spatiotemporal effect remain in their usual places in the linear predictor.
For delta models, $x_t$ is shared but each component has its own SVC field $\zeta_{m,\s}$.
The index is jointly estimated and is recomputed from simulated spatiotemporal fields during simulation.

The mean $\bar{\epsilon}_{tm}$ is an unweighted arithmetic mean over mesh vertices, so the index can depend on mesh construction.
The SVC represents an endogenous association between the model-derived index and spatial redistribution; it is not automatically evidence of a causal density-dependent mechanism.
Other interpretations can include changing spatial distributions with overall incidence, prevalence, abundance, productivity, exposure, or activity.
This construction is motivated by the density-dependent species-distribution model of Thorson (2022).

One direct `model_index()` term is supported.
It must exactly match a complete fixed-effect term in every model component and be constant within each fitted time.
At least two fitted times and an explicit `time` argument are required.
Prediction and simulation are supported at fitted times; `extra_time` and forward `project()` are not.
...
For `time_lag(x)`, the transformed covariate follows a temporal recursion at each vertex:

$$
\tilde{x}_{\s,1} = x_{\s,1}, \qquad
\tilde{x}_{\s,t} = x_{\s,t} + \kappa_{T}\tilde{x}_{\s,t-1} \quad (t > 1).
$$

Lag-scale parameters are estimated separately for each covariate included in `nonlocal_formula`.
`time_lag()` terms require a `time` argument in `sdmTMB()`.

For spatial diffusion terms, the model output also reports `MSD[...]` and `RMSD[...]`, the implied mean squared displacement and root mean squared displacement.
With the current parameterization, `\mathrm{MSD} = 4/\kappa_S^2` and `\mathrm{RMSD} = 2/\kappa_S`; `RMSD` is a typical diffusion distance (a blur radius) in coordinate units, where larger values imply broader smoothing and smaller values imply more local diffusion.
...
Thorson, J. T. (2022). Development and simulation testing for a new approach to density dependence in species distribution models. *ICES Journal of Marine Science*, 79, 117--128. https://doi.org/10.1093/icesjms/fsab247
