...
**This feature is experimental.** It is available only with the RTMB backend, via `sdmTMB(..., preferential = preferential_sampling(...))`, and currently supports single log-link families (Poisson, NB2, Gamma, Tweedie, lognormal) and conventional logit/log `delta_gamma()` and `delta_lognormal()` families.
