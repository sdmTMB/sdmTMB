...
This is a newer, experimental feature, enabled via the `preferential_grid`, `preferential_response`, and `preferential_b_type` arguments to `sdmTMBcontrol()`.

The sampling sub-model reuses a restricted version of the main model's linear predictor -- fixed effects plus the spatial and spatiotemporal random fields only (smooths, time-varying coefficients, spatially varying coefficients, random intercepts/slopes, and nonlocal covariate terms are not included) -- projected from mesh vertices onto the sampling grid with its own projection matrix $\bA_{\RM{pref}}$:

$$
\eta^{*}_{\s,t} = \bX^{\RM{pref}}_{\s,t} \bbeta + \bA_{\RM{pref}} \bomega + \bA_{\RM{pref}} \beps_t,
$$

where $\bX^{\RM{pref}}_{\s,t}$ is the fixed-effect design matrix built from the same `formula` but evaluated on the sampling grid, and $\bbeta$ is the same fixed-effect coefficient vector as the main model.
