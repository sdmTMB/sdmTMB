...
Covariate diffusion terms [@lindmark2025; @thorson2026] can be added with the `covariate_diffusion` argument to `sdmTMB()` as a one-sided formula using `space()` and/or `time()` wrappers.
For example: `~ space(x) + time(x)`.
...
`time()` terms require a `time` argument in `sdmTMB()`.
