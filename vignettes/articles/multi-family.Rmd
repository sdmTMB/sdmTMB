...
| `plot_smooth()` | Not supported | Use `visreg`, direct `predict()` calls on custom grids, or other custom plotting workflows. |
| `spread_sims()` / `gather_sims()` | Not supported | Use `predict(..., nsim = ...)` or `simulate()` draws directly. |
| `do_index = TRUE` inside `sdmTMB()` | Not supported | Fit first, then call `predict(..., return_tmb_object = TRUE)` and pass that to `get_index()`. |
