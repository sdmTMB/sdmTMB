...
fit <- sdmTMB(
  formula = list(
    ...
    response ~ factor(year) + poly(log_depth, 2) # dropping factor(data_type) here, see note above
  ),
  data = dat,
  family = family_list,
...
- **Link space**: Standard errors are always in link space. For non-multi-family models,
  `type = "response"` with `se_fit = TRUE` will warn and return predictions in link space.
