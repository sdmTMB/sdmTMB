...
The original `dogfish` data are tow-level points. To fit CAR/SAR, we first define polygon areal units (grid cells) and assign each tow to a cell. We keep the tow-level observations for model fitting so that the model sees the original catch, effort, and tow-specific covariates.
...
To align supports directly, we predict the SPDE model at grid-cell center locations for the same observed tow rows used by the CAR/SAR fits. We then summarize fitted values to `cell_id`-`year` rows for maps and model-comparison plots.
...
For continuous data, a geostatistical SPDE model is a more natural approach and it better fits the data here. However, sometimes with very large datasets, there may be a computational advantage to aggregating to a grid and using a CAR or SAR model. That aggregation trades speed for loss of tow-level information. Even without aggregation, the CAR model followed by the SAR model were faster than the SPDE model (approximately twice as fast in this case).
