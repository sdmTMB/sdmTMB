...
This vignette demonstrates a simple spatial covariate-diffusion model with simulated data.
We will simulate a fine-scale predictor observed at sample locations and a spatially diffused effect.
Then we will fit a model that estimates the diffused covariate from the fine-scale predictor.
...
simulate_spatial_diffusion <- function(
  ...
  # Apply the same spatial covariate-diffusion operator used in the model:
...
## Fit a covariate-diffusion model
...
To fit a covariate diffusion model with spatial diffusion, we add the one sided formula `~ space(x1)` to the `covariate_diffusion` argument.
Users applying this spatial diffusion method should cite @lindmark2025.

In other contexts, we could add a time-diffused predictor with `time(x1)` and a diffused predictor in which the spatial diffusion changes with time with `space(x1) + time(x1) + spacetime(x1)`.
Users applying the time and space-time diffusion extensions should cite @thorson2026.

We will omit the spatial random field in this simple example.
In other more realistic scenarios, you may choose to estimate both.
...
Here we see `kappaS_dl[x1]`, which is the estimated spatial diffusion scale parameter for `x1` (larger values imply less smoothing).

We also see `MSD[x1]` and `RMSD[x1]` which are the implied mean squared displacement (`4 / kappaS_dl[x1]^2`) and root mean squared displacement (`2 / kappaS_dl[x1]`).
Intuitively, `RMSD[x1]` is a typical diffusion distance (a blur radius) in the same units as the coordinates.
Larger `RMSD[x1]` means the covariate is smoothed over longer distances, while smaller `RMSD[x1]` means diffusion is more local.
`MSD[x1]` contains the same information on a squared-distance scale.
Users using these MSD and RMSD metrics should consult and cite @thorson2026.

## Plot the estimated diffusion kernel

We can plot the diffusion kernel:

```{r plot-diffusion}
plot_covariate_diffusion(fit, component = "space")
```

## Compare true and estimated diffused covariates

Compare the known true diffused covariate with our estimated one.
We wouldn't normally know the truth.
