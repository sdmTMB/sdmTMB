...
Cross-validation is one of the best approaches for quantifying model performance and comparing sdmTMB models with different structures (unlike AIC, this approach also accounts for uncertainty in random effects).
Arguably the most challenging decision when implementing cross-validation is how to specify the folds (each fold representing a subset of data that is, in turn, held out and used as a test set).
Folds may vary in number and how data are partitioned, and will likely be slightly different for each application.

The goals of some sdmTMB applications may be focused on spatial prediction; these include predicting to new spatial regions (e.g. unsampled areas or areas not sampled in every year).
...
Alternatively, the goal of an analysis with sdmTMB may be to evaluate the predictive accuracy of a model through time (e.g. for a missing survey year, or for prediction to future years).
...
Cross-validation in sdmTMB is implemented using the `sdmTMB_cv()` function, with the `k_folds` argument specifying the number of folds (defaulting to 8).
The function uses parallelization by default if a `future::plan()` is set, but this can be turned off with the `parallel` argument.
...
In the above example, folds are assigned randomly, but these can be modified for specific spatial or temporal applications.
...
Or similarly, these clusters could be assigned in time, here with each year assigned to a unique fold. Note that year is not included as a factor and spatiotemporal fields are turned off because they cannot be estimated in missing years.
...
Many measures of predictive accuracy can be used to evaluate model performance.
By default, `sdmTMB_cv()` returns a list that contains the sum of the log-likelihood values for each left-out fold and the total summed across the left-out folds. This is analogous to the expected log predictive density (ELPD) in the Bayesian literature and can be interpreted as a measure of predictive ability for new observations.
...
The `fold_loglik` values represent the sum of predictive log-likelihood values across observations for each fold, and the `sum_loglik` value represents the sum of `fold_loglik` across the entire dataset. We can also calculate derived statistics based on the predictions, here using Root Mean Square Error (RMSE) and Mean Absolute Error (MAE) as examples.
...
In cases where only a single test set is evaluated (e.g., 10% of the data), using the `sdmTMB_cv()` function may be overkill because two `sdmTMB()` models will be fit, but using this function may still be worthwhile to reduce coding errors in the log-likelihood calculations.
...
We can ignore the total log-likelihood, and just focus on the first element of the list:
...
For example, if we wanted to evaluate support for a depth effect, we could do 10-fold cross-validation (it is important that the folds be the same across the two models).
...
Finally, instead of identifying a single "best" model, we may be interested in model averaging.
...
If instead we had split the data into a 10/90 split (as in the example above), we would not want to use the second model fit to generate these weights.
...
A commonly used diagnostic is the AUC (area under the curve), which quantifies the ability of a model to discriminate between the two classes; this is computed from the Receiver Operating Characteristic (ROC) curve, which plots the true positive rate versus the false positive rate.
...
AUC may be sensitive to class imbalance, however, and alternative metrics may better reflect predictive skill.
...
The waywiser package [@mahoney_2023] provides tools for assessing spatial models, including methods for evaluating model performance at multiple spatial scales.
...
Because prediction errors can be spatially distributed and may compound or counteract each other when aggregated, it is useful to assess model performance across multiple scales.
