...
We will start with some data simulated from scratch. We will simulate from an NB2 negative binomial observation model with a spatial random field, an intercept, and one predictor named `a1` that has a linear effect on the observed data.
...
The second model will match the simulated data.
...
We can see just by looking at these fits that the Poisson model inflates the spatial random-field standard deviation (SD) relative to the truth.
The model missing the `a1` predictor does so to a lesser degree.

## Analytical randomized-quantile residuals

Here are randomized quantile residuals with fixed effects fixed at their MLEs (maximum likelihood estimates) and random effects taken from a single sample of their approximate distribution (more details below):
...
They apply randomization to integer response values, transform the residuals with the relevant distribution function so that they should follow a uniform(0, 1) distribution under the model, and then optionally transform those values so that they should follow a normal(0, 1) distribution.
...
The above approach assumes that a draw of the random effects can be approximated by a multivariate normal distribution. If we want to relax that assumption, we can sample the random effects with MCMC while holding the fixed effects at their MLEs.
...
These return a matrix where each row represents an observation and each column is a simulation draw:
...
There are clearly too few zeros in the data simulated from the Poisson model, whereas the NB2 model seems reasonable.
...
In the QQ residual plots we clearly see evidence of overdispersion relative to the Poisson model.
Note the values clumping near 1.0 on the observed axis and deviating downwards towards 0.0 observed.
This is indicative of too many zeros and variance increasing too rapidly with the mean for the Poisson distribution, resulting in some large outlying values.
...
We can see a trend in the residuals against `a1` because we omitted it from the model.
...
We can extract the DHARMa residuals if we want to plot them ourselves. For example, let's extract the DHARMa residuals from a model and make a QQ plot for each year:
...
Indeed, our test (incorrectly) rejects the null hypothesis that $r_1 \sim \operatorname{N}(0, 1)$, even though if the residuals are calculated correctly we know they do come from $\operatorname{N}(0, 1)$.

If instead we return to a single sample from the assumed MVN random-effect distribution, we get the "correct" residuals:
...
We could also sample that draw of random effects using MCMC (with the fixed effects still held at their MLEs), which relaxes our assumptions, but is much more time-intensive for large models.
...
We get back to something with the wrong reference distribution for comparison.
...
Let's simulate data again, but with large observation error (`phi` here, which is the Gaussian error SD in this case) and smaller random-field variances (`sigma_E` and `sigma_O`):
...
Now, it does not really matter because the "incorrect" random-effect distribution is swamped by the observation-error contribution to the distribution.
Technically, the first set is "wrong" and the second set is "right", but functionally we would come to a similar conclusion in this case.
...
Both are valid, and which to use is largely a matter of preference, but it is important to appreciate how this changes the appearance of the expected residuals.
...
The normal(0, 1) residuals put more emphasis on the tails. This is both good and bad: it is easier to examine tail behaviour, but these residuals can often look "off" even when the model is fine (as in this example) because observations in the tails of the distribution are, by definition, rarely observed.
...
Either is valid, but you do need to adjust your expectations accordingly. For example, poor tail behaviour may look like a minor issue with uniform residuals; conversely, the tails of normal(0, 1) residuals are unlikely ever to look "perfect" without large sample sizes.
...
This works for any distribution as long as we can define the distribution function.
...
The gamma example above illustrated how we can use the distribution function (there `pgamma()`) to take a gamma-distributed variable and turn it into a uniformly distributed variable.

Now we need to do the same with the Poisson equivalent: `ppois()`.
The `ppois(y)` function gives us the cumulative probability up to value `y`.
...
But if we naively apply `ppois()` to the observed values, we end up with discrete cumulative probabilities that are not very useful for comparing against a continuous uniform distribution.

```{r}
hist(ppois(y, mu))
```

Instead, we need to get the cumulative probability up to the value below the one we observed and "fill in" the values up to the observed value with the desired uniformly distributed samples.
First, get the cumulative probability up to the value below and up to the observed value:
