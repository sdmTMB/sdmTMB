...
By default, `project()` samples estimated parameter uncertainty and the
historical latent states, then simulates future spatiotemporal and
time-varying effects. To keep those uncertainty sources while propagating
dynamic future effects at their conditional means, use
`sample_future_re = FALSE`:
