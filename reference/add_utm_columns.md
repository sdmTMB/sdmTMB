...
performed in a projected coordinate reference system with distance-based
units. You can do this yourself with the
...
- utm_crs:
  ...
  Output CRS value for the UTM zone; tries to detect it with `get_crs()`
...
You may wish to work in km rather than the standard UTM meters so that
the range parameter estimate is not too small, which can cause
computational issues. This depends on the scale of your data.
