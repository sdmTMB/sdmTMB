...
tidy(
  x,
  effects = c("fixed", "ran_pars", "ran_vals", "ran_vcov", "rsr", "dispersion"),
  model = 1,
  conf.int = TRUE,
  conf.level = 0.95,
  exponentiate = FALSE,
...
- effects:
  ...
  `ranef()`), `"ran_vcov"` (list of variance covariance matrices for the
  random effects, by model and group), or `"rsr"` (Restricted Spatial
  Regression fixed-effect coefficients adjusted for spatial confounding
  with the random fields; Hanks et al. 2015; Diaz and Thorson 2025). To
  access RSR coefficients the model must be fitted with
  `control = sdmTMBcontrol(get_rsr = TRUE)`. `"dispersion"` returns
  dispersion-model coefficients when `dispformula` is used.
...
(e.g., `phi`) only when dispersion is scalar. With `dispformula`, use
`effects = "dispersion"` to extract dispersion-model coefficients.
