...
- seed:
  ...
  Random number seed. Simulations use the backend the model was fit
  with; TMB and RTMB fits give different values for the same seed.
...
- model:
  ...
  combined, `1` = first model, `2` = second model.
...
- re_form:
  ...
  random effects (smoothers, which internally are random effects, will
