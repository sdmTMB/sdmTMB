...
- obj:
  ...
  with `return_tmb_object = TRUE` is also accepted.

- newdata:

  New data (e.g., a prediction grid by year) to pass to
  [`predict.sdmTMB()`](https://sdmTMB.github.io/sdmTMB/reference/predict.sdmTMB.md).
  Not used when `obj` is legacy prediction output.
...
- offset:
  ...
  An optional numeric offset vector with one value per row of `newdata`.
...
- predict_args:
  ...
  A named list of less commonly used arguments to pass to
  [`predict.sdmTMB()`](https://sdmTMB.github.io/sdmTMB/reference/predict.sdmTMB.md).
  `newdata` and `offset` should be supplied directly.
