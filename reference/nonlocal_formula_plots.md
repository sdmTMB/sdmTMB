...
- component:
  ...
  `"time_lag"`, or `"combined"`. `"combined"` plots the fitted joint
  operator and requires both wrappers for `covariate`.
...
slices. When both `diffusion()` and `time_lag()` were fitted for the
same covariate, `component = "combined"` visualizes the single
stationary joint operator represented by that covariate's transformed
prediction column.
