...
- nonlocal_formula:
  ...
  `~ diffusion(x) + time_lag(x)`. Note that diffused spatial covariates
  will be held constant across time slices unless the `time` argument is
  specified. See the [non-local covariates
