...
- area:
  ...
  A vector of grid cell/polygon areas for each year-grid cell (row of
...
- area_function:
  ...
  Function to apply area weighting. Assuming a log link, the default
  `function(x, area) x + log(area)` makes sense. If in natural space,
  `function(x, area) x * area` makes sense.

- agg_function:

  Function to aggregate samples within each time slice. Assuming a log
  link, the default `function(x) sum(exp(x))` makes sense. If in natural
...
- the name of the time column supplied to
