...
Example fish survey datasets used throughout the package documentation
and examples.
...
`qcs_grid`: A 2 x 2 km prediction grid for Queen Charlotte Sound. A data
frame.
...
`hbll_s_grid`: A survey domain grid corresponding to `yelloweye`. A data
frame.

`wcvi_grid`: A survey domain grid corresponding to `dogfish`. A data
frame.
