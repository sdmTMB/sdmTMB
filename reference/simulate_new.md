...
- B:
  ...
  followed by covariate-diffusion coefficients in formula term order.
...
- nonlocal_formula:
  ...
  pass to
  [`sdmTMB()`](https://sdmTMB.github.io/sdmTMB/reference/sdmTMB.md).
  Supported wrappers are `diffusion()` and `time_lag()`.
...
- lags_rhoT:
  ...
  Temporal diffusion persistence for `time_lag()` terms. Must be finite
  and satisfy `0 <= rhoT < 1`. Supply a single value or one value per
  covariate needing temporal diffusion.
