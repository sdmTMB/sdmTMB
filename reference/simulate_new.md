...
`sdmTMB_simulate()` is retained as a synonym for backwards
compatibility. We recommend using `simulate_new()` going forward as it
more clearly conveys the function's purpose. `sdmTMB_simulate()` may
...
- B:
  ...
  `covariate_diffusion` is used, include regular fixed-effect
  coefficients followed by covariate-diffusion coefficients in formula
  term order.
...
- covariate_diffusion:
  ...
  pass to
  [`sdmTMB()`](https://sdmTMB.github.io/sdmTMB/reference/sdmTMB.md).
  Supported wrappers are `space()` and
  [`time()`](https://rdrr.io/r/stats/time.html).

- diffusion_kappaS:

  Spatial diffusion scale for `space()` terms. Must be positive and
  finite. Supply a single value or one value per covariate needing a
  spatial scale.

- diffusion_rhoT:

  Temporal diffusion persistence for
  [`time()`](https://rdrr.io/r/stats/time.html) terms. Must be finite
